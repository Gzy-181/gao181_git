...
{
    if(__HAL_UART_GET_FLAG(&uart_handle,UART_FLAG_RXNE)!=RESET)//������ռĴ�����Ϊ��
    {
        uint8_t receive_data=0;
        HAL_UART_Receive(&uart_handle,&receive_data,1,1000);//�������ݲ��ŵ����ݻ�����
        usart_rx_buff[rx_cnt++]=receive_data;
        
    }
     if(__HAL_UART_GET_FLAG(&uart_handle, UART_FLAG_IDLE) != RESET)//���������ж�
    {
        printf("recv: %s\r\n", usart_rx_buff);
        uart_clear();
        __HAL_UART_CLEAR_IDLEFLAG(&uart_handle);//��������жϱ�־λ
