...
class WeatherStationApp:
    ...
    def create_widgets(self):
        ...
        baud_label = tk.Label(serial_frame, text="波特率:", bg="#f0f8ff")
        ...
        temp_label.grid(row=0, column=0, padx=15, pady=10, sticky=tk.W)
        self.temp_value = tk.Label(grid_frame, text=f"{self.temperature}℃", font=("Arial", 14, "bold"), bg="#f0f8ff", fg="#e74c3c")
        self.temp_value.grid(row=0, column=1, padx=15, pady=10, sticky=tk.W)
        
        # 湿度
        hum_label = tk.Label(grid_frame, text="湿度:", font=("Arial", 12), bg="#f0f8ff")
        hum_label.grid(row=1, column=0, padx=15, pady=10, sticky=tk.W)
        self.hum_value = tk.Label(grid_frame, text=f"{self.humidity}%", font=("Arial", 14, "bold"), bg="#f0f8ff", fg="#3498db")
        self.hum_value.grid(row=1, column=1, padx=15, pady=10, sticky=tk.W)
        
        # 光照强度
        light_label = tk.Label(grid_frame, text="光照强度:", font=("Arial", 12), bg="#f0f8ff")
        light_label.grid(row=0, column=2, padx=15, pady=10, sticky=tk.W)
        self.light_value = tk.Label(grid_frame, text=f"{self.light} lx", font=("Arial", 14, "bold"), bg="#f0f8ff", fg="#f39c12")
        self.light_value.grid(row=0, column=3, padx=15, pady=10, sticky=tk.W)
        
        # 气压
        pressure_label = tk.Label(grid_frame, text="气压:", font=("Arial", 12), bg="#f0f8ff")
        pressure_label.grid(row=1, column=2, padx=15, pady=10, sticky=tk.W)
        self.pressure_value = tk.Label(grid_frame, text=f"{self.pressure} hPa", font=("Arial", 14, "bold"), bg="#f0f8ff", fg="#27ae60")
        self.pressure_value.grid(row=1, column=3, padx=15, pady=10, sticky=tk.W)
        
        # 降水等级
        rain_label = tk.Label(grid_frame, text="降水等级:", font=("Arial", 12), bg="#f0f8ff")
        rain_label.grid(row=2, column=0, padx=15, pady=10, sticky=tk.W)
        self.rain_value = tk.Label(grid_frame, text=self.rain_level, font=("Arial", 14, "bold"), bg="#f0f8ff", fg="#9b59b6")
        self.rain_value.grid(row=2, column=1, padx=15, pady=10, sticky=tk.W)
    ...
    def connect_serial(self):
        ...
        try:
            ...
            print(f"尝试连接到串口 {port}，波特率 {baud_rate}")  # 调试信息
            # 明确设置串口参数，确保与单片机一致
    ...
    def parse_data(self, line):
        ...
        if light_match:
            ...
            print(f"解析到光照强度: {self.light} lx")  # 调试信息
            self.update_display()
        ...
        if temp_pressure_match:
            ...
            print(f"解析到温度: {self.temperature}℃, 气压: {self.pressure} hPa")  # 调试信息
            self.update_display()
        ...
        if press_match:
            ...
            print(f"解析到气压: {self.pressure} hPa")  # 调试信息
            self.update_display()
        ...
        if rain_match:
            ...
            print(f"解析到降水等级: {self.rain_level}")  # 调试信息
            self.update_display()
    ...
    def update_display(self):
        ...
        def update():
            ...
            print(f"界面已更新: 温度={self.temperature}℃, 湿度={self.humidity}%, 光照={self.light} lx, 气压={self.pressure} hPa, 降水等级={self.rain_level}")  # 调试信息
